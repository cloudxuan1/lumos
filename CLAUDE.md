...
| 7 | sleeping | 💤 | 在睡觉 | BED |
| 8 | study | 💭 | 在思考 | THINKING |
| 9 | peril | ⚠️ | 性命攸关 | MORTAL PERIL |

钟面一圈只显示**英文花体字**（沿弧弯排，底部自动翻正保证可读），不显示 emoji；emoji + 中文在下方控制卡里。注意 `study` 的内部 key 不变（DB / lumos-api 枚举仍是 `study`），只是显示文案改成 THINKING / 在思考 / 💭。后续可加自定义状态增删改，第一版先固定这九个。
