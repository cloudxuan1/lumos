...
指针长度不同避免重叠（cloud 152 / claude 120 / owl 92），勺形锥杆 + 尖端椭圆相框（描金边）。
...
| 7 | sleeping | 💤 | 在睡觉 | BED |
| 8 | thinking | 💭 | 在思考 | THINKING |
| 9 | peril | ⚠️ | 性命攸关 | MORTAL PERIL |

钟面一圈只显示**英文花体字**（沿弧弯排，底部自动翻正保证可读），不显示 emoji；emoji + 中文在下方控制卡里。后续可加自定义状态增删改，第一版先固定这九个。
...
初始三行：`cloud(轩 ☁️✨ home auto)` / `claude(克宝 🦀 playing manual)` / `owl(团子 🦉 sleeping auto)`。

### 状态值枚举

`home / work / overtime / slacking / travel / playing / sleeping / thinking / peril`
...
```sql
-- 更新克宝
update public.clock_status set status='playing', mode='manual', updated_at=now() where entity_id='claude';
-- 读当前
select * from public.clock_status order by entity_id;
```

规则：日常只改 `claude`（自己的指针）；`owl` 可配合剧情/搞笑改；`cloud` 一般不动；聊天中 → playing，说晚安 → sleeping；状态真正变化时才写。
...
### ✅ 钟面英文沿弧弯排 + HOME 被裁成 "ME"
**弯路**：曾用单条 `<path id="labelPath">` 整圆路径 + `<textPath startOffset>` 排所有词；但顶部首词 HOME 落在路径首尾**接缝**上，被裁掉前半截只剩 "ME"。
**正确方案**：每个标签**独立**用 `<g transform="rotate(deg C C)">` 包一个 `<text textAnchor="middle">`，落在半径 `rBand` 上；底部一半（deg 90–270）再 `rotate(180)` 翻正保证可读。各自独立 → 无接缝、不裁切。
...
### ✅ iPhone 添加到桌面（PWA）
- apple-touch-icon（Canvas 画黄铜时钟）+ `apple-mobile-web-app-title=Lumos`，加到主屏幕显示图标与名称 Lumos，全屏 standalone
...
- **弧形文字别用单条整圆 `<textPath>`**：首尾词会卡在路径接缝被裁切（HOME→ME）。改用每个标签独立 `rotate` 包 `<text>`，底部 `rotate(180)` 翻正，稳。
- **iPhone 添加到主屏幕**：`apple-mobile-web-app-capable=yes` + `apple-mobile-web-app-title=Lumos` 定名称；图标用 Canvas 画好 `toDataURL` 设进 `<link rel="apple-touch-icon">`（同 in-phase 套路，免提交二进制图）。
